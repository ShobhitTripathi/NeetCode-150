...
class Solution {
    ...
    public ListNode reverseKGroup(ListNode head, int k) {
        // Create a dummy node to simplify edge case handling (e.g., reversing the first group)
        ListNode dummy = new ListNode(0, head);
        // Pointer to track the node before the current group
        ListNode groupPrev = dummy;

        while (true) {
            // Get the k-th node from the current group's starting node
            ListNode kth = getKth(groupPrev, k);
            // If there are fewer than k nodes remaining, exit the loop
            if (kth == null) {
                break;
            }
            // Store the next node after the current group
            ListNode groupNext = kth.next;

            // Reverse the current group
            ListNode prev = groupNext; // Start with the next group's head
            ListNode curr = groupPrev.next; // Current node to reverse

            while (curr != groupNext) {
                // Reverse the links within the group
            ...
            ListNode tmp = groupPrev.next; // Save the original start of the group
            groupPrev.next = kth;         // Connect the previous group to the new start
     ...
     * @param k    The number of steps to move forward
     * @return The k-th node, or null if there are fewer than k nodes remaining
     */
    private ListNode getKth(ListNode curr, int k) {
        ...
        // Return the k-th node or null if insufficient nodes
        return curr;
    }
}
