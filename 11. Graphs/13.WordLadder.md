...
Solution 2-:
Approach:
We use Breadth-First Search (BFS) to find the shortest transformation sequence from beginWord to endWord.
To optimize, we precompute intermediate generic patterns for each word (e.g., "d*g", "*og", etc.), which allows O(1) lookup of neighbors (i.e., words that differ by only one letter).

Example:
For the word dog, the generic patterns are:
*og
d*g
do*

Using these patterns, we construct a map of pattern → words, which helps efficiently identify one-letter transformations during BFS.
...
BFS traversal: O(N * L), since each word has up to L patterns.

Space Complexity:
O(N * L) for the pattern map and visited set
*/

public class Solution {
    public int ladderLength(String beginWord, String endWord, List<String> wordList) {
        // If endWord is not in the list, we cannot reach it
        if (!wordList.contains(endWord)) {
            return 0;
        }

        // Map from pattern (like d*g or do*) to all words matching that pattern
        Map<String, List<String>> nei = new HashMap<>();

        // Add beginWord to wordList to ensure it's included in graph
        wordList.add(beginWord);

        // Preprocess all words and build pattern -> words map
        for (String word : wordList) {
            for (int j = 0; j < word.length(); j++) {
                // Replace each character with '*' to generate intermediate pattern
        ...
        Queue<String> q = new LinkedList<>();
        q.offer(beginWord);
        int res = 1; // Initial transformation count includes beginWord
        ...
        while (!q.isEmpty()) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                // Found the endWord - return the number of transformations
                if (word.equals(endWord)) {
                    return res;
                }

                // Explore all neighbors by pattern matching
                for (int j = 0; j < word.length(); j++) {
                    String pattern = word.substring(0, j) + "*" + word.substring(j + 1);
                    for (String neiWord : nei.getOrDefault(pattern, Collections.emptyList())) {
                        if (!visit.contains(neiWord)) {
                            ...
                            q.offer(neiWord);    // Add to queue for next level
                        }
                    }
                }
            }
            res++; // Increment level (number of transformations)
        }
