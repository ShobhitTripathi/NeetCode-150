...
class Solution {
    ...
    List<Integer> output; // Stores the course order
    Set<Integer> visited; // Tracks visited nodes
    Set<Integer> visiting; // Tracks nodes in the current recursion stack to detect cycles
   
    public int[] findOrder(int numCourses, int[][] prerequisites) {
        graph = new HashMap<>(); // Adjacency list to represent the graph
        output = new ArrayList<>(); // Stores the course order
        visited = new HashSet<>(); // Tracks visited nodes
        visiting = new HashSet<>(); // Tracks nodes in the current recursion stack to detect cycles
    ...
    private boolean dfs(int course) {
       if (visited.contains(course)) { // Already processed
            return true;
        }
        if (visiting.contains(course)) { // Cycle detected
            return false;
        }

        visiting.add(course); // Mark the course as being processed

        // Process all prerequisites for the current course
        for (int pre : graph.getOrDefault(course, Collections.emptyList())) {
            if (!dfs(pre)) {
                return false; // Return false if a cycle is detected in prerequisites
            }
        }

        visitng.remove(course); // Remove from recursion stack
        visited.add(course); // Mark the course as fully processed
