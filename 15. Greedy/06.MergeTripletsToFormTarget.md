...
Approach
```
Filter out invalid triplets (any value exceeding the target),
then check whether across the remaining triplets
we can cover all three indices where at least one triplet matches the target value in that position;

if all three positions are covered, merging is possible.
```
